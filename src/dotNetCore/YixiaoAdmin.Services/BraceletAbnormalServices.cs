...
namespace YixiaoAdmin.Services
{
    ...
    /// 手环异常记录服务实现
    /// </summary>
    public class BraceletAbnormalServices : BaseServices<BraceletAbnormal>, IBraceletAbnormalServices
    {
        private IBraceletAbnormalRepository _braceletAbnormalRepository;
        
        public BraceletAbnormalServices(IBraceletAbnormalRepository repository)
        {
            this._braceletAbnormalRepository = repository;
            base.baseRepository = _braceletAbnormalRepository;
        }
        ...
        public async Task<PagesResponse> QueryPages(QueryPageModel queryPageModel)
        {
            ...
            //初始化查询表达式
            Expression<Func<BraceletAbnormal, bool>> whereExpression = PredicateBuilder.True<BraceletAbnormal>();
            //判断是否存在查询条件
            if (queryPageModel.Query != null)
            {
                ...
                {
                    //根据属性名获取属性
                    var property = typeof(BraceletAbnormal).GetProperty(item.QueryField);
                    if (property == null)
                    {
                        continue;
                    }
                    ...
                    {
                        whereExpression = PredicateBuilder.And(whereExpression, (x) => x.WorkerName.Contains(item.QueryStr));
                    }
            ...
            pagesResponse.Success((await _braceletAbnormalRepository.Query(whereExpression, queryPageModel.Orderby, queryPageModel.CurrentPage, queryPageModel.PageNumber)).ToList());
            pagesResponse.count = (await _braceletAbnormalRepository.Query(whereExpression)).Count();
