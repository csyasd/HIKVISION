...
<script>
import { SelectWorkRecord, SelectWorkOrder, SelectALLDevice } from '@/api/api.js'

export default {
  name: 'MobileWorkBracelet',
  data() {
    return {
      DeviceList: [],
      WorkOrderList: [],
  ...
  methods: {
    ...
    async getTableData() {
      ...
      const workOrderIds = filtered.length ? filtered.map(wo => wo.Id) : []
      const Query = workOrderIds.length
        ? [{ QueryField: 'WorkOrderId', QueryStr: workOrderIds.join(',') }]
        : [{ QueryField: 'WorkOrderId', QueryStr: '__NO_MATCH__' }]
      try {
        const res = await SelectWorkRecord({
          Query,
        ...
        allData.forEach(r => {
          ...
          if (!byWorker[r.WorkerName] || new Date(r.CreateTime) > new Date(byWorker[r.WorkerName].CreateTime)) {
            byWorker[r.WorkerName] = r
          }
        })
