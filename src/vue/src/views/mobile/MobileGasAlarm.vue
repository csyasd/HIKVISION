  ...
  <div class="mobile-list-page">
    ...
    <div v-loading="loading" class="list-wrap">
      <div v-for="(row, i) in tableData" :key="row.Id || i" class="list-card">
        ...
        <div class="card-row">
          <span class="label">甲烷</span>
          <span>{{ (row.Gas3 || 0).toFixed(2) }} %LEL</span>
        </div>
        <div class="card-row">
          <span class="label">二氧化硫</span>
...
<script>
import { SelectGasAlarmRecord, SelectWorkOrder, SelectALLDevice } from '@/api/api.js'
...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      try {
        const res = await SelectGasAlarmRecord({
          Query,
        ...
        const latestByWorkOrder = {}
        allData.forEach(r => {
          if (!r.WorkOrderId) return
          if (!latestByWorkOrder[r.WorkOrderId] || new Date(r.CreateTime) > new Date(latestByWorkOrder[r.WorkOrderId].CreateTime)) {
            latestByWorkOrder[r.WorkOrderId] = r
          }
        })
        const arr = Object.values(latestByWorkOrder)
        this.totalNumber = arr.length
        const start = (this.currentPage - 1) * this.pageSize
        this.tableData = arr.slice(start, start + this.pageSize)
      } catch (e) {
